...
class Retriever:
    ...
    def add_passages(self, passages, top_passages_and_scores):
        ...
        docs = [passages[doc_id]["text"] for doc_id in top_passages_and_scores[0][0]]
        return docs

    def setup_retriever(self):
        print(f"Loading model from: {self.args.model_name_or_path}")
        if self.args.model_name_or_path ==  "facebook/contriever-msmarco":
            self.model, self.tokenizer, _ = src.contriever.load_retriever(self.args.model_name_or_path)
        elif self.args.model_name_or_path == "facebook/dragon-plus-query-encoder":
            self.tokenizer =  AutoTokenizer.from_pretrained(self.args.model_name_or_path)
            self.model =  AutoModel.from_pretrained(self.args.model_name_or_path)
        else:
            raise Exception("error setup_retriever !")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--query",
        type=str,
        default="Can you tell me the difference between llamas and alpacas?",
